...
bool PortableReadFileToProto2(const std::string& file_name,
 ...
 * <!--  IsAsset2():  -->
 */
bool IsAsset2(const char* const filename) {
 ...
 * <!--  ReadFileToProto2():  -->
 */
void ReadFileToProto2(AAssetManager* const asset_manager,
                     const char* const filename,
                     google::protobuf::MessageLite* message) {
	
	if (!IsAsset2(filename)) {
		VLOG(0) << "Opening file: " << filename;
		CHECK(PortableReadFileToProto2(filename, message));
		return;
	}
